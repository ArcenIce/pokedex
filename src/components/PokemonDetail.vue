<template>
  <div class="detail" v-if="loaded && visible">
    <div class="detail-view card">
      <div class="data card-body">
        <img class="image" :src="imgs + pokemon.name + '.png'">
        <h2 class="card-title">{{pokemon.name}}</h2>
        <h3>Types:</h3>
        <div class="types" v-for="item in pokemon.types" :key="item.slot">
          <div class="type">
            <span :class="item.type.name">{{ item.type.name }}</span>
          </div>
        </div>
        <h3>Talents:</h3>
        <div class="abilities" v-for="item in pokemon.abilities" :key="item.ability.name">
          <div class="ability">{{ item.ability.name }}</div>
        </div>
        <div class="property">
          <div class="left bold">Taille:</div>  <div class="right">{{pokemon.height}}</div>
        </div>
        <div class="property">
          <div class="left bold">Poids:</div>  <div class="right">{{pokemon.weight}}</div>
        </div>
      </div>
